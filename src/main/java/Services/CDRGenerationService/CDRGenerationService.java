...
public class CDRGenerationService {
     ...
     *  */
    private void generateCDR(Long startTime, int numOfFile) throws IOException {
        Random rand = new Random();
    ...
    private @NotNull List<String> getRandomMSISDNs(int amount) throws RuntimeException{
        ...
        try {
            for (int i = 1; i <= amount; i++) {
                String number = dbManager.getMSISDN();
                if (!number.equals("")) {
                    numbers.add(number);
                } else throw new RuntimeException();
            }
        }  catch (SQLException e) {
            System.out.println("Cannot generate random numbers. Check connection to database");
        }
        return numbers;
    }
